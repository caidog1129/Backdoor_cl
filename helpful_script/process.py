...
def evenly_distribute_folders(directory):
    ...
    folders_per_instance = len(folders) // 4

    for i in range(4):
        start_idx = i * folders_per_instance
        end_idx = (i + 1) * folders_per_instance if i < 3 else None
        instance_folders = folders[start_idx:end_idx]

        instance_file_path = f"instance_{i + 1}.txt"  # Define the path for the instance.txt file in the current directory
